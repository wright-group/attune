    ...
    def fit(self, *args, **kwargs):
        ...
        **kwargs
        ...
        if self.dimensionality == 1:
            args = tuple(wt.kit.remove_nans_1D(args))
            if len(args[0]) == 0:
...
class Gaussian(Function):
    ...
    def guess(self, values, xi):
        ...
        list of numbers
            Guessed parameters.
        """
        values, xi = wt.kit.remove_nans_1D([values, xi])
        if len(values) == 0:
            return [np.nan] * 4
...
class TwoD_Gaussian(Function):
    ...
    def _Format_input(self, values, x, y):
        ...
        try:
            v, xi, yi = wt.kit.remove_nans_1D([v, xi, yi])
            return v, xi, yi, True
        except BaseException:
            return np.nan, np.nan, np.nan, False
...
class Fitter:
    ...
    def __init__(self, function, data, *args, **kwargs):
        ...
        **kwargs
        ...
        self.data = data.copy()
        self.data.transform(*args)

        axis_shape = wt.kit.joint_shape(*self.data.axes)
        
        self.fit_shape = tuple(o//n for o, n in zip(self.data.shape, axis_shape))
    ...
    def run(self, channel=0, propagate_other_channels=True, verbose=True):
        ...
        # transpose data --------------------------------------------------------------------------
        # Transposing taken out because it no longer means anything in WT3, but probably need
        # to do _something_ here...
        # create output objects -------------------------------------------------------------------
        for a in self.data.axis_names:
            self.data.collapse(a, method='integrate')
        params_channels = []
        for param in self.function.params:
            values = np.full(self.data.shape, np.nan)
            self.data.create_channel(param, values, units=None)

        # do all fitting operations ---------------------------------------------------------------
        axes_points = [axis[:] for axis in self.data.axes]
        timer = wt.kit.Timer(verbose=False)
        with timer:
...
class MultiPeakFitter:
    ...
    def __init__(self, data, channel=0, name='', fittype=2, intensity_label='OD'):
        ...
        self.diff = wt.kit.diff(self.data.axes[0].points, self.zi, order=self.fittype)
    ...
    def fit(self, verbose=True):
        ...
        zi_diff = wt.kit.diff(self.data.axes[0].points,
    ...
    def plot(self,):
        ...
        cm = wt.artists.colormaps['default']
        # create figure
        fig, gs = wt.artists.create_figure(width='single', cols=[1], nrows=2, default_aspect=.5)
        # as-taken
    ...
    def save(self, path=os.getcwd(), fit_params=True, figure=True, verbose=True):
        ...
        if fit_params:
            ...
            write = wt.kit.write_headers(params_path, headers)
            if verbose:
                print('Parameters saved to:', write)
        if figure:
            fig = self.plot()
            fig_path = os.path.join(path, ' '.join((self.name, timestamp.path, 'fits.png')))
            write = wt.artists.savefig(fig_path, fig=fig, close=True)
            if verbose:
